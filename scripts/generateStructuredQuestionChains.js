...
// 文件路径
const classStructurePath = path.resolve(__dirname, '../src/data/class_structure.json');
const analyzeSentimentPath = path.resolve(__dirname, '../src/data/analyze_sentiment.json');
const outputPath = path.resolve(__dirname, '../src/data/structured_question_chains.json');
...
function processStructuredQuestionChains(classStructure, questions) {
  ...
  // 获取第二级child（教学环节）
  function extractTeachingPhases(node, currentTime = 0) {
    const phases = [];
    
    if (node.child) {
      for (const child of node.child) {
        const beginTime = child.beginTime !== undefined ? child.beginTime : currentTime;
        
        // 如果有子节点，递归获取
        if (child.child && child.child.length > 0) {
          const subPhases = extractTeachingPhases(child, beginTime);
